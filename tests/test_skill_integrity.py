...
class SkillIntegrityTests(unittest.TestCase):
    ...
    def test_stage2_docs_keep_agent_judgment_outside_fetch_scripts(self) -> None:
        ...
        self.assertIn("references/stage2a-data-intake.md", skill_text)
        self.assertIn("references/stage2b-scholarship-map.md", skill_text)
        ...
        self.assertIn("中文主题下优先用于首轮发现与补漏", intake_text)
        self.assertIn("读取 `referenced_works` 并抓取这些 works", intake_text)
        self.assertIn("完成两个 API 入口的检查与检索", intake_text)
        self.assertIn("不是“首轮 `OpenAlex` 后的网页补检”", intake_text)
        ...
        self.assertIn("只搜了 1 到 2 轮", scholarship_text)
