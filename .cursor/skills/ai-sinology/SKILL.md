...
- 阶段三：先读 `references/stage3a-deepened-thinking.md`；进入检索收敛时再读 `references/stage3b-data-intake.md`；进入学术史地图写作时再读 `references/stage3c-scholarship-map.md`（`3C` 每次启动前须先完成 PDF→Markdown 前置检查，见该文件）。
...
- 阶段三 `3C` 启动时，将 `papers/` 中 PDF 转为 Markdown 时优先复用 `scripts/stage3c_mineru_pdfs.py`（默认 `vlm`，与仓库根目录 `test_mineru_v4_batch.py` 同源 API 流程），不要每次现场重写 MinerU 调用。
- 阶段三脚本（`stage3b_sources.py`、`stage3c_mineru_pdfs.py`）的 API 密钥：默认合并 **仓库根目录 `.env`** 与 **`.cursor/skills/ai-sinology/.env`**（后者覆盖同名键）；可只把技能相关 key 放在技能目录 `.env`（仍被全局 `.env` 规则忽略，勿提交）。
...
- 新建项目时，同时创建 `outputs/<project>/project_progress.yaml`、`outputs/<project>/_stage3b/papers/` 与 `outputs/<project>/_stage3c/papers_md/`。
- 每推进一个阶段，都更新 `project_progress.yaml`；优先使用 `scripts/sync_progress.py`。
- 阶段二过程文件统一放在 `outputs/<project>/_stage2/`。
- 阶段三：`3B` 用 `outputs/<project>/_stage3b/`；`3C` 的 MinerU 全文在 `outputs/<project>/_stage3c/papers_md/`（见 `stage3c-scholarship-map.md`、`scripts/stage3c_mineru_pdfs.py`）。
...
- 阶段三需要进入 `3C`，但 `candidate_papers.md` 或 `papers/` 中人工补料尚未满足要求，或 `papers/` 中已有 PDF 但尚未在 `_stage3c/papers_md/` 生成对应的 `*_mineru.md`（且仓库根与技能目录 `.env` 均未配置 `MINERU_API_TOKEN` / `MINERU_TOKEN` 等条件无法转换）。
...
python3 .agent/skills/ai-sinology/scripts/stage3b_sources.py --project demo openalex --query "汉代 灾异 诠释"
python3 .agent/skills/ai-sinology/scripts/stage3b_sources.py --project demo openalex-expand --query "汉代 灾异 诠释" --seed-id W123 --expand-mode references
uv run python .cursor/skills/ai-sinology/scripts/stage3c_mineru_pdfs.py demo
