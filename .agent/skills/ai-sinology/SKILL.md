...
- 如果阶段二以 `OpenAlex` 为主入口，默认先做关键词检索，再由 agent 阅读返回结果、挑出值得继续追踪的作品，沿这些作品的引用链继续扩展；也可以并行调用 `baidu-scholar` 作为中文题录与摘要补充源。首轮 `OpenAlex` 后，还应使用 agent 自身的网页搜索/浏览能力补检，优先整理期刊官网、DOI 落地页、出版社页面、机构知识库与可引用数据库落地页，不把普通博客或聚合站当作依据。
