...
- 阶段一：读 `references/stage1-planning.md`。
  选刊时再读 `references/stage1_journals_intro.md`，先用其中的路由部分初筛，再精读对应期刊小节；如果需要按国内 A 刊标准推进，再加读 `references/a-journal-writing.md`。
...
- 总原则：
  ...
  - 遇到较长文件时，允许分多轮、分段持续写入，不要求一轮内完整写完；优先保证每一轮都能稳定落盘，避免因单轮写太多而中途中断。
...
- 阶段一：
  ...
  - 如果用户没有说清投稿目标，先追问。
  - 如果用户点名的目标期刊不在现有内置期刊介绍内，优先请用户提供该刊官网、征稿说明、投稿须知或近年栏目页链接；先据此提炼要求，再继续阶段一。
  - 阶段一只负责收束研究方向、投稿约束与阶段二入口，不替代后续一手材料整理与学术史判断。
  - 阶段一必须给阶段二写出明确的检索主题，优先放进 YAML front matter 的 `stage2_retrieval_themes`。
  - 阶段一在正文解释这些检索主题时，应使用带序号的列表，便于后续阶段二粗筛按相同顺序映射；但 front matter 中的 `stage2_retrieval_themes` 仍保持纯主题文本，不把 `1.`、`T1` 之类编号写进值本身。
  - `1_research_proposal.md` 末尾必须补一个精简的“史料目录建议”部分，结合当前主题，判断最应优先看的四库全书目录、四部丛刊目录与 Kanripo 文件夹名称；如本地 `data/kanripo_repos` 可用，可做少量核验，但不要把 proposal 写成目录说明书。
...
- 阶段二：
  ...
  - 阶段二粗筛为了节省 token，只输出主题序号到 `T/F` 的 JSON 映射，例如 `{"1":"T","2":"F"}`；运行时会按阶段一主题顺序还原成完整主题，再进入后续精筛与仲裁。
  - 正式阶段文件是 `2_primary_corpus.yaml`；如有运行时配置，额外保留 `outputs/<project>/_stage2/manifest.json` 与 `outputs/<project>/_stage2/`。
...
- 阶段二完成后，项目里至少要有 `outputs/<project>/2_primary_corpus.yaml`；如已生成 `outputs/<project>/_stage2/manifest.json`，一并保留。
