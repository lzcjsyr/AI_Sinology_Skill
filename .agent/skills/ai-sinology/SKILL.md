...
- 阶段二：
  ...
  - 正式阶段文件是 `2_primary_corpus.yaml`；如有运行时配置，额外保留 `outputs/<project>/_stage2/2_stage2_manifest.json` 与 `outputs/<project>/_stage2/`。
...
- 阶段二完成后，项目里至少要有 `outputs/<project>/2_primary_corpus.yaml`；如已生成 `outputs/<project>/_stage2/2_stage2_manifest.json`，一并保留。
