...
from repo_helpers import ensure_workspace_on_syspath, find_workspace_root, normalize_scope_token


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="列出或校验 Kanripo 可用 scope。")
...
def split_tokens(raw_value: str) -> list[str]:
    ...
    return [normalize_scope_token(token) for token in normalized.split(",") if token.strip()]
...
def main() -> int:
    ...
    if not kanripo_root.exists():
        raise SystemExit("缺少 data/kanripo_repos。")
    ...
    if args.validate:
        ...
        for token in split_tokens(args.validate):
            ...
            family = normalize_scope_token(family_match.group(1)) if family_match else None
